...
class FalAIClient:
    ...
    async def generate_image(self, model_id: str, payload: BaseModel) -> FalImageGenerationOutput:
        """
        ...
        try:
            ...
            log.debug("Determined base model ID for polling", full_id=model_id, base_id=base_model_id)
            
            json_payload = payload.model_dump(exclude_none=True)

            submit_response = await self._client.post(f"{self._base_url}/{model_id}", json=json_payload)
            ...
            for attempt in range(45):
                ...
                log.debug("Polling Fal.run job", request_id=request_id, status=status_data.status, attempt=attempt + 1)

                if status_data.status == "COMPLETED":
                    log.info("Job completed, fetching final result.", request_id=request_id)
                    ...
                    try:
                        ...
                        raise FalAIError("Fal.run returned an invalid 'COMPLETED' response structure.") from e

                if status_data.status in ["FAILED", "ERROR"]:
                    log.error("Fal.run job failed", request_id=request_id, status_data=status_data.model_dump())
                    raise FalAIError(f"Image generation job failed with status: {status_data.status}")
