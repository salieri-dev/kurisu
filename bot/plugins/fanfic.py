...
async def fanfic_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        await message.reply_text("❌ Пожалуйста, укажите тему для фанфика: `/fanfic [тема]`", quote=True)
        return
    
    topic = message.text.split(maxsplit=1)[1]
    if len(topic) < 3:
        await message.reply_text("❌ Тема слишком короткая! (минимум 3 символа)", quote=True)
        return
